...
/* ── helpers ───────────────────────────────────────────── */
const KPI_COLORS = {
  total:        { bg: 'bg-[#0f1f3d]', border: 'border-[#1e3a6e]', accent: '#3b82f6', label: 'text-blue-300' },
  storage:      { bg: 'bg-[#0d2137]', border: 'border-[#1a4060]', accent: '#06b6d4', label: 'text-cyan-300' },
  exam:         { bg: 'bg-[#1a0d38]', border: 'border-[#3b1f6e]', accent: '#a855f7', label: 'text-purple-300' },
  unidentified: { bg: 'bg-[#2a1a00]', border: 'border-[#6b3d00]', accent: '#f59e0b', label: 'text-amber-300' },
  flagged:      { bg: 'bg-[#2a0d0d]', border: 'border-[#6b1f1f]', accent: '#ef4444', label: 'text-red-300' },
  released:     { bg: 'bg-[#0a2218]', border: 'border-[#145c39]', accent: '#22c55e', label: 'text-green-300' },
};

function KpiCard({ label, value, icon: Icon, colorKey, to, delta }) {
  const c = KPI_COLORS[colorKey];
  const inner = (
    <div className={`relative rounded-2xl border ${c.bg} ${c.border} p-5 flex flex-col gap-4 group hover:brightness-110 transition-all`}>
      <div className="flex items-center justify-between">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: c.accent + '22', border: `1px solid ${c.accent}44` }}>
          <Icon className="w-5 h-5" style={{ color: c.accent }} />
        </div>
        {delta !== undefined && (
          <span className={`text-[11px] font-medium flex items-center gap-0.5 ${delta >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <div>
        ...
        <p className={`text-xs font-medium mt-1.5 uppercase tracking-wider ${c.label}`}>{label}</p>
      </div>
      <div className="absolute bottom-0 left-0 h-0.5 w-full rounded-b-2xl" style={{ background: `linear-gradient(to right, ${c.accent}88, transparent)` }} />
    </div>
  );
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-[#0f1f3d] border border-[#1e3a6e] rounded-lg px-3 py-2 text-xs text-white shadow-xl">
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#070f1e] text-white">
      {/* Top header */}
      <div className="border-b border-[#1e3a6e]/60 bg-[#0a1628] px-6 py-4">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              <Activity className="w-3.5 h-3.5 text-blue-400/60" />
              <p className="text-[11px] text-blue-400/60 font-mono uppercase tracking-widest">Operations Intelligence Center</p>
            </div>
            <h1 className="text-lg font-bold text-white">Custiviant Dashboard</h1>
          ...
          <div className="flex items-center gap-3">
            {!loading && todayIntakes.length > 0 && (
              <div className="flex items-center gap-2 bg-green-500/10 border border-green-500/30 rounded-full px-3 py-1.5">
                ...
                <span className="text-xs text-green-300 font-medium">{todayIntakes.length} new intake{todayIntakes.length > 1 ? 's' : ''} today</span>
              </div>
            )}
            <Link to="/intake">
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid xl:grid-cols-3 gap-5">
          ...
          <div className="xl:col-span-2 bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-sm font-semibold text-white">Intake & Release Trend</h2>
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={monthlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#1e3a6e44" />
          ...
          {/* Pie — Status Distribution */}
          <div className="bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-2">
                  {statusPieData.map((d, i) => (
                    <div key={d.name} className="flex items-center gap-1.5">
                      ...
                      <span className="text-[11px] font-semibold text-white ml-auto">{d.value}</span>
                    </div>
                  ))}
                </div>
              </>
        ...
        <div className="grid xl:grid-cols-3 gap-5">
          ...
          <div className="bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-sm font-semibold text-white">Examination Types</h2>
                <p className="text-xs text-slate-400 mt-0.5">All time breakdown</p>
              </div>
              <FlaskConical className="w-4 h-4 text-purple-400" />
            </div>
            {loading || examBarData.length === 0 ? (
              <div className="h-44 bg-white/5 rounded-xl animate-pulse flex items-center justify-center">
                <p className="text-xs text-slate-500">No examination data</p>
              </div>
            ) : (
          ...
          <div className="bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-sm font-semibold text-white">Identification Status</h2>
                  ...
                  {idPieData.map((d, i) => {
                    ...
                    return (
                      <div key={d.name} className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-slate-400 flex-1">{d.name}</span>
          ...
          {/* Live Activity Feed */}
          <div className="bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div>
              ...
              <Link to="/audit" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-0.5">
                View all <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
            <div className="flex-1 space-y-1 overflow-hidden">
              {custodyLogs.length === 0 ? (
                <p className="text-xs text-slate-500 text-center py-6">No activity logged</p>
              ) : custodyLogs.map(log => (
                <div key={log.id} className="flex items-start gap-2.5 px-2 py-2 rounded-lg hover:bg-white/5 transition-colors">
                  <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: actionTypeColors[log.action_type] || '#64748b' }} />
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-200 capitalize leading-snug">
        ...
        <div className="grid xl:grid-cols-3 gap-5">
          ...
          <div className="xl:col-span-2 bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/intake-list" className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-0.5">
                View all <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <Link to="/intake"><Button size="sm" variant="outline" className="mt-3 gap-2 border-slate-700 text-slate-300"><UserPlus className="w-3.5 h-3.5" /> First Intake</Button></Link>
              </div>
            ) : (
              <div className="divide-y divide-[#1e3a6e]/30">
                {recentCases.map(d => {
                  const name = d.first_name ? `${d.first_name} ${d.last_name || ''}`.trim() : 'Unidentified';
                  return (
                    <Link key={d.id} to={`/decedent/${d.id}`} className="flex items-center gap-3 py-3 hover:bg-white/5 px-2 -mx-2 rounded-lg transition-colors group">
                      <div className="w-8 h-8 rounded-full bg-[#1e3a6e] flex items-center justify-center shrink-0">
                        <span className="text-xs font-bold text-blue-300">{name[0]}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-[10px] text-slate-500 bg-white/5 px-1.5 py-0.5 rounded">{d.unique_id}</span>
                          {d.flags?.length > 0 && <AlertTriangle className="w-3 h-3 text-amber-400" />}
                        </div>
                        <p className="text-sm font-medium text-slate-200 truncate mt-0.5">{name}</p>
                      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#0a1628] border border-[#1e3a6e]/50 rounded-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-white flex items-center gap-1.5">
                  <LogOut className="w-4 h-4 text-amber-400" /> Pending Releases
                </h2>
